...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::Title { message: "Title cannot be empty".to_string()});
        }
        if title.len() > 50 {
            return Err(TicketNewError::Title { message: "Title cannot be longer than 50 bytes".to_string()});
        }
